...
class Exllamav2HF(PreTrainedModel):
    def __init__(self, config: ExLlamaV2Config):
        ...
        if True:
            split = None
            if '':
                split = [float(alloc) for alloc in ''.split(",")]

            self.ex_model.load(split)
        self.past_seq = None
        self.ex_cache = ExLlamaV2Cache(self.ex_model, lazy=False)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        ...
        pretrained_model_name_or_path = Path('models/') / Path(pretrained_model_name_or_path)
        ...
        config.num_experts_per_token = int(2) # 2
